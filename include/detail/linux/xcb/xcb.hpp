...
namespace xcb {
    ...
    public:
        ...
        void notify_window_property_change(Window window, Atom property, Atom target, Atom selection) {
            ...
            xcb_send_event(m_conn.get(), false, window, XCB_EVENT_MASK_PROPERTY_CHANGE, reinterpret_cast<const char*>(&event));
            xcb_flush(m_conn.get());
        }
        ...
        std::string get_our_property_value(Atom property) {
            xcb_get_property_cookie_t cookie = xcb_get_property(m_conn.get(), static_cast<uint8_t>(true), m_window, property,
                    XCB_ATOM_ANY, 0, -1);
        ...
        std::unique_ptr<Event> convert_generic_event_to_event(std::unique_ptr<xcb_generic_event_t> event) {
            ...
            switch (event_type) {
                ...
                case XCB_SELECTION_REQUEST: {
                    xcb_selection_request_event_t* sel_request_event = reinterpret_cast<xcb_selection_request_event_t*>(event.get());
                    return std::make_unique<RequestSelectionEvent>(sel_request_event->requestor, sel_request_event->owner,
                        sel_request_event->selection, sel_request_event->target, sel_request_event->property);
                }
                ...
                case XCB_SELECTION_NOTIFY: {
                    xcb_selection_notify_event_t* sel_notify_event = reinterpret_cast<xcb_selection_notify_event_t*>(event.get());
                    return std::make_unique<SelectionNotifyEvent>(sel_notify_event->requestor,
                        sel_notify_event->selection, sel_notify_event->target, sel_notify_event->property);
                }
